...
import { type DealEntity, type TaskEntity, type NoteEntity, type DealActivity } from '@/domains/deals/api/dealsApi';
...
export function ChatMessages({ messages, isLoading }: ChatMessagesProps) {
  ...
  // Render Deal Activities Timeline
  const renderActivitiesList = (activities: DealActivity[]) => {
    if (!activities || activities.length === 0) {
      return <div className="text-xs text-foreground/40 italic p-2 bg-muted rounded-xl">No activity logs found for this deal.</div>;
    }
      ...
      <div className="space-y-3 mt-3">
        ...
        <div className="relative pl-3 border-l-2 border-border/80 ml-2 space-y-3">
          {activities.map((activity) => {
            ...
            const noteContent = activity.noteId?.content || activity.objectData?.Note?.content;
            ...
            if (markdownDescription) {
              ...
              switch (action) {
                ...
                case "STAGE_UPDATE":
                  ...
                  content = (
                    <>
                      <strong>{actorName}</strong> moved the stage from <strong>{previousStageName || "Unknown"}</strong> to <strong>{stageName || "Unknown"}</strong>.
                    </>
                  );
                  break;
                case "STATUS_CREATE":
                  ...
                  content = (
                    <>
                      <strong>{actorName}</strong> created status <strong>{statusName || "Unknown"}</strong>.
                    </>
                  );
                  break;
                case "TAG_CREATE":
                case "TAG_UPDATE":
                  ...
                  content = (
                    <>
                      <strong>{actorName}</strong> {previousTag ? (
                        <>updated the tag from <strong>{previousTag}</strong> to <strong>{currentTag || "Unknown"}</strong>.</>
                      ) : (
                ...
                case "REASSIGN":
                  ...
                  content = (
                    <>
                      <strong>{actorName}</strong> reassigned the deal from <strong>{previousAssignee || "Unassigned"}</strong> to <strong>{currentAssignee || "Unassigned"}</strong>.
                    </>
                  );
                  break;
                case "WALK_IN":
                  ...
                  content = (
                    <>
                      Deal was created as a <strong>walk-in</strong> by <strong>{actorName}</strong>.
                    </>
                  );
                  break;
                default:
                  ...
                  content = (
                    <>
                      <strong>{actorName}</strong> performed <strong>{action || "an activity"}</strong>.
                    </>
                  );
            ...
            const time = formatActivityTime(activity.activityDate || activity.createdAt);
                ...
                <div className="bg-card border border-border p-2.5 rounded-xl shadow-sm space-y-1">
                  ...
                  {comment && (
                    <div className="rounded-lg border border-border/50 bg-muted/20 px-2 py-1.5 mt-1">
                      <p className="text-[11px] text-foreground/55 leading-relaxed whitespace-pre-wrap">{comment}</p>
                    </div>
                  )}
                </div>
              </div>
  ...
  const renderToolsOutput = (toolsData: Message['toolsData']) => {
    ...
    switch (toolName) {
      ...
      case 'list_deal_activities':
        return renderActivitiesList(data);
      default:
        return null;
    }
  };
...
function getActivityActorName(activity: DealActivity) {
  if (activity.actorId && typeof activity.actorId === "object") {
    return `${activity.actorId.firstName || ""} ${activity.actorId.lastName || ""}`.trim();
  }
